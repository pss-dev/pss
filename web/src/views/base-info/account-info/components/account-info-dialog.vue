  ...
  <el-dialog
    ...
    :before-close="handleClose">
      <el-form :model="accountData" ref="accountData" :rules="rules" class="demo-ruleForm">
        <el-form-item label="账户编号" prop="accountID" :label-width="formLabelWidth">
          <el-input v-model="accountData.id" autocomplete="off"></el-input>
        </el-form-item>
...
<script>
  export default {
    ...
    methods:{
      ...
      submitData() {
        this.$refs['accountData'].validate((valid) => {
          if (valid) {
            this.$emit("submitData", this.accountData, this.oldID);
          } else {
    ...
    created: function() {
      if(this.accountData) {
        this.oldID = this.accountData.id;
      }
    }
  }
</script>
